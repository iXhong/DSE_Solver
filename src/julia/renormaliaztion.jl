...
const m0 = 0.005 #GeV 
const ξ = 361.0 # GeV
const γ_m = 12.0 / 25.0 #(33.0 - 2.0 * Nf)
const τ = exp(2.0) - 1.0
...
function IntegralOfAx(p2::Float64, q2::Float64)::Float64
    function integrand(x::Float64)::Float64
        ...
        return part2 * G(k2) * (sqrt(p2 * q2) * x + part1 / k2)
    end
...
# Iterative solver
function iterate_AB(Z2::Float64, Z4::Float64, A::Vector{Float64}, B::Vector{Float64}; tol::Float64=1.0, max_iter::Int=100)
    ...
    for iteration in 1:max_iter
        Threads.@threads for i in 1:N
            ...
            A_prime[i] = 1.0 / (6.0 * p2 * π^3) * IA # 改了
            A_new[i] = Z2 + A_prime[i] - 1 
            B_prime[i] = 1.0 / (2.0 * π^2) * IB
            B_new[i] = Z4 * m0 + B_prime[i] - m0 
        end
...
function findZ2Z4(A::Vector{Float64}, B::Vector{Float64})
    ...
    Z4 = 1.0 - B_prime[xi_index] / m0
    println("A(ξ) = $(A[xi_index]), expected ≈ 1")
    println("B(ξ) = $(B[xi_index]), expected ≈ $m0")
    return Z2, Z4, A, B
end
